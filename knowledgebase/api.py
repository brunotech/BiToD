...
def is_equal_to(value):
    return value if is_mongo else (lambda x: x == value)
        
def is_not(value):
    return {"$ne": value} if is_mongo else (lambda x: x != value)
...
def contains_none_of(value):
    if is_mongo:
        ...
        return lambda x: all(e not in x for e in value)


def is_one_of(value):
    return {"$in":value} if is_mongo else (lambda x: x in value)


def is_at_least(value):
    return {"$gte": value} if is_mongo else (lambda x: x >= value)


def is_less_than(value):
    return {"$lt": value} if is_mongo else (lambda x: x < value)

def is_at_most(value):
    return {"$lte": value} if is_mongo else (lambda x: x <= value)
...
dbs = {
    "null": None,
    "restaurants_en_US_booking": mydb["restaurants_en_US"],
    "restaurants_en_US_search": mydb["restaurants_en_US"],
    "hotels_en_US_search": mydb["hotels_en_US"],
    "hotels_en_US_booking": mydb["hotels_en_US"],
    "attractions_en_US_search": mydb["attractions_en_US"],
    "weathers_en_US_search": mydb["weathers_en_US"],
    "restaurants_zh_CN_booking": mydb["restaurants_zh_CN"],
    "restaurants_zh_CN_search": mydb["restaurants_zh_CN"],
    "hotels_zh_CN_search": mydb["hotels_zh_CN"],
    "hotels_zh_CN_booking": mydb["hotels_zh_CN"],
    "attractions_zh_CN_search": mydb["attractions_zh_CN"],
    "weathers_zh_CN_search": mydb["weathers_zh_CN"],
}
is_mongo = True
...
def restaurants_en_US_booking(api_name,db,query,api_out_list=None):
    ...
    api_return = {k: results[-1][k] for k in api_out_list} | pre_api_return
...
def call_api(api_name, constraints: List[Dict[Text, Any]]) -> Tuple[Dict[Text, Any], int]:
    ...
    if api_name in ["restaurants_en_US_search","restaurants_en_US_booking", "hotels_en_US_search", "hotels_en_US_booking", "attractions_en_US_search", "weathers_en_US_search"]:
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "apis", f"{api_name}.json"), "r") as file:
